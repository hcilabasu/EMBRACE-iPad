...
- (void)playCurrentSentenceAudio {
    ...
    if (parentManipulaitonCtr.conditionSetup.reader == SYSTEM || (parentManipulaitonCtr.conditionSetup.appMode == ITS && parentManipulaitonCtr.conditionSetup.useKnowledgeTracing && parentManipulaitonCtr.stepContext.numSyntaxErrors > 0)) {
        ...
        if ([parentManipulaitonCtr.chapterTitle isEqualToString:@"Cleaning Up"] && ([parentManipulaitonCtr.pageContext.currentPageId containsString:PM1] || [parentManipulaitonCtr.pageContext.currentPageId containsString:PM2])) {
            sentenceAudioFile = [NSString stringWithFormat:@"CleaningUpS%dE.mp3", parentManipulaitonCtr.sentenceContext.currentSentence];
        }
